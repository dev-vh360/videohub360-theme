...
// Determine if user is in business mode (professional, organization, or client)
$is_business_mode = in_array($account_type, array('professional', 'organization', 'client'), true);
    ...
    if (!wp_verify_nonce($_POST['vh360_edit_profile_nonce'], 'vh360_edit_profile_action')) {
        ...
        if (empty($display_name)) {
            ...
            if (is_wp_error($result)) {
                ...
                // Handle cover image upload (skip in business mode)
                if (!$is_business_mode && !empty($_FILES['cover_image']['name'])) {
