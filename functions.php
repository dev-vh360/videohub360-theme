 ...
 * 
 * Smart reuse logic:
 * - Checks if location is already assigned (preserves customizations)
 * - Checks if a menu with the expected name already exists (reuses imported menus)
 * - Only creates new menu if neither condition is met
 * 
 * This ensures imported demo menus are reused rather than duplicated.
 *
 * @since 1.0.0
 */
function vh360_create_default_menus() {
    ...
    if ( empty( $existing_locations['dashboard'] ) ) {
        ...
        if ( $menu_obj && ! is_wp_error( $menu_obj ) ) {
            // Menu already exists (e.g., from demo import) - reuse it
            $dashboard_menu_id = (int) $menu_obj->term_id;
        } else {
            // Create new menu
            $dashboard_menu_id = wp_create_nav_menu( $menu_name );
        }
        
        if ( ! is_wp_error( $dashboard_menu_id ) ) {
            // Only populate if menu has no items (empty or just created)
            $existing_items = wp_get_nav_menu_items( $dashboard_menu_id );
    ...
    if ( empty( $existing_locations['vh360_mobile_bottom'] ) ) {
        ...
        if ( $menu_obj && ! is_wp_error( $menu_obj ) ) {
            // Menu already exists (e.g., from demo import) - reuse it
            $mobile_bottom_menu_id = (int) $menu_obj->term_id;
        } else {
            // Create new menu
            $mobile_bottom_menu_id = wp_create_nav_menu( $menu_name );
        }
        
        if ( ! is_wp_error( $mobile_bottom_menu_id ) ) {
            // Only populate if menu has no items (empty or just created)
            $existing_items = wp_get_nav_menu_items( $mobile_bottom_menu_id );
            if ( empty( $existing_items ) ) {
    ...
    if ( empty( $existing_locations['vh360_mobile_drawer'] ) ) {
        $menu_name = 'Mobile User Drawer';
        $menu_obj = wp_get_nav_menu_object( $menu_name );
        
        if ( $menu_obj && ! is_wp_error( $menu_obj ) ) {
            // Menu already exists (e.g., from demo import) - reuse it
            $mobile_drawer_menu_id = (int) $menu_obj->term_id;
        } else {
            // Create new menu
            $mobile_drawer_menu_id = wp_create_nav_menu( $menu_name );
        }
        
        if ( ! is_wp_error( $mobile_drawer_menu_id ) ) {
            // Only populate if menu has no items (empty or just created)
            $existing_items = wp_get_nav_menu_items( $mobile_drawer_menu_id );
            if ( empty( $existing_items ) ) {
    ...
    // Save location assignments if any were modified
