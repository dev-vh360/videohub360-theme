...
function vh360_giving_default_options(){ return array('enable_giving'=>0,'dashboard_tab_label'=>'My Giving','checkout_display_name'=>'','default_currency'=>'usd','minimum_amount'=>'1','suggested_amounts'=>'10,25,50,100','enable_anonymous'=>1,'enable_notes'=>1,'success_message'=>'Thank you for your gift.','cancel_message'=>'Your giving checkout was canceled.'); }
function vh360_giving_options(){ return wp_parse_args(get_option('vh360_giving_options',array()), vh360_giving_default_options()); }
function vh360_giving_checkout_display_name(){
    $options = vh360_giving_options();
    $custom_name = isset($options['checkout_display_name']) ? trim(wp_strip_all_tags((string) $options['checkout_display_name'])) : '';
    if ('' !== $custom_name) {
        return $custom_name;
    }
    ...
    if ('' !== $site_name) {
        return sprintf(
            ...
            __('%s Giving', 'videohub360-memberships'),
            $site_name
        );
    }
    return __('Giving', 'videohub360-memberships');
}
