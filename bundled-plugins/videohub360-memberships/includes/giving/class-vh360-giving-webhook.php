...
class VH360_Giving_Webhook {
    public static function maybe_handle_event($event_type,$object,$event_id){
        ...
        if('checkout.session.expired'===$event_type) return self::handle_checkout_expired($object,$event_id);
    ...
    private static function handle_checkout_expired($session,$event_id){
        $type=$session['metadata']['type']??'';
        if('giving'===$type){
            $tx_id=absint($session['metadata']['transaction_id']??0);
            $tx=VH360_Giving_Transactions::get($tx_id);
            if(!$tx) return new WP_Error('giving_tx_missing','Giving transaction not found.');
            if('paid'===$tx->status) return true;
            if('pending'===$tx->status){
                VH360_Giving_Transactions::update($tx_id,array('status'=>'expired','stripe_checkout_session_id'=>sanitize_text_field($session['id']??''),'gateway_event_id'=>$event_id,'stripe_event_id'=>$event_id,'gateway_customer_id'=>sanitize_text_field($session['customer']??''),'stripe_customer_id'=>sanitize_text_field($session['customer']??'')));
            }
            return true;
        }
        if('giving_recurring'===$type){
            $recurring_id=absint($session['metadata']['recurring_id']??0);
            $gift=VH360_Giving_Recurring::get($recurring_id);
            if(!$gift) return new WP_Error('giving_recurring_missing','Recurring giving record not found.');
            if('incomplete'===$gift->status) VH360_Giving_Recurring::mark_canceled($recurring_id);
            return true;
        }
        return false;
    }
