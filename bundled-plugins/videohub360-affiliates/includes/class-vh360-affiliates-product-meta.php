...
class VH360_Affiliates_Product_Meta {
    ...
    public function render_meta_box($post) {
        ...
        $enabled    = get_post_meta($post->ID, '_vh360_affiliate_enabled',         true);
        $use_global = get_post_meta($post->ID, '_vh360_affiliate_use_global',      true);
        $type       = get_post_meta($post->ID, '_vh360_affiliate_commission_type', true) ?: 'percentage';
        ...
        <p>
            <label>
                <input type="checkbox" name="vh360_affiliate_enabled" value="1" <?php checked('1', $enabled); ?>>
                <?php esc_html_e('Enable affiliate commissions', 'videohub360-affiliates'); ?>
            </label>
        </p>
        <p id="vh360-aff-commission-settings">
            <label>
                <input type="checkbox" name="vh360_affiliate_use_global" value="1" <?php checked('1', $use_global); ?>>
                <?php esc_html_e('Use global commission settings', 'videohub360-affiliates'); ?>
            </label>
        </p>
        <p>
        ...
        <p>
            ...
            <input type="number" name="vh360_affiliate_commission_rate" value="<?php echo esc_attr($rate); ?>" step="0.01" min="0" style="width:100%">
        </p>
        <?php
    }
    ...
    public function save_meta_box($post_id, $post) {
        ...
        update_post_meta($post_id, '_vh360_affiliate_enabled',
            isset($_POST['vh360_affiliate_enabled']) ? '1' : '');
        update_post_meta($post_id, '_vh360_affiliate_use_global',
            isset($_POST['vh360_affiliate_use_global']) ? '1' : '');
        update_post_meta($post_id, '_vh360_affiliate_commission_type',
            isset($_POST['vh360_affiliate_commission_type'])
                ? sanitize_key(wp_unslash($_POST['vh360_affiliate_commission_type']))
                : 'percentage');
        update_post_meta($post_id, '_vh360_affiliate_commission_rate',
            isset($_POST['vh360_affiliate_commission_rate'])
    ...
    public function add_variation_fields($loop, $variation_data, $variation) {
        $enabled    = get_post_meta($variation->ID, '_vh360_affiliate_enabled',         true);
        $use_global = get_post_meta($variation->ID, '_vh360_affiliate_use_global',      true);
        $type       = get_post_meta($variation->ID, '_vh360_affiliate_commission_type', true) ?: 'percentage';
        ...
        echo '<label><input type="checkbox" name="vh360_aff_var_exclude[' . esc_attr($loop) . ']" value="1" ' . checked('1', $exclude, false) . '> ' . esc_html__('Exclude variation', 'videohub360-affiliates') . '</label><br>';
        echo '<label><input type="checkbox" name="vh360_aff_var_enabled[' . esc_attr($loop) . ']" value="1" ' . checked('1', $enabled, false) . '> ' . esc_html__('Enable override', 'videohub360-affiliates') . '</label><br>';
        echo '<label><input type="checkbox" name="vh360_aff_var_use_global[' . esc_attr($loop) . ']" value="1" ' . checked('1', $use_global, false) . '> ' . esc_html__('Use global settings', 'videohub360-affiliates') . '</label><br>';
    ...
    public function save_variation_fields($variation_id, $loop) {
        ...
        update_post_meta($variation_id, '_vh360_affiliate_enabled',
            isset($_POST['vh360_aff_var_enabled'][$loop]) ? '1' : '');
        update_post_meta($variation_id, '_vh360_affiliate_use_global',
            isset($_POST['vh360_aff_var_use_global'][$loop]) ? '1' : '');
        update_post_meta($variation_id, '_vh360_affiliate_commission_type',
            isset($_POST['vh360_aff_var_type'][$loop])
                ? sanitize_key(wp_unslash($_POST['vh360_aff_var_type'][$loop]))
                : 'percentage');
        update_post_meta($variation_id, '_vh360_affiliate_commission_rate',
            isset($_POST['vh360_aff_var_rate'][$loop])
