(function () {
    ...
    function schedulePreviewFrame() {
        if (previewFrame || !previewContext) { return; }
    ...
    function fitText(context, text, maxWidth, startSize, minSize, weight) {
        ...
        do {
            ...
            if (context.measureText(text).width <= maxWidth || size <= minSize) { return size; }
            size -= 2;
        } while (size > minSize);
        return minSize;
    }
    ...
    function geometry(context, config, frame) {
        ...
        const primarySize = fitText(context, primary, maxWidth * 0.82, frame.height * 0.042 * scale, frame.height * 0.026, '800');
        const secondarySize = secondary ? fitText(context, secondary, maxWidth * 0.82, frame.height * 0.026 * scale, frame.height * 0.018, '600') : 0;
        const paddingX = frame.width * 0.018 * scale;
        const paddingY = frame.height * 0.016 * scale;
        const textWidth = Math.max(context.measureText(primary).width, secondary ? context.measureText(secondary).width : 0);
        ...
        return { x, y, width, height, paddingX, primarySize, secondarySize, primary, secondary };
    }
    ...
    function drawConfig(context, config, frame, alpha, offsetX) {
        ...
        context.font = '800 ' + g.primarySize + 'px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
        context.fillText(g.primary, textX, g.y + g.paddingY);
        if (g.secondary) {
            context.fillStyle = style.secondaryColor || '#dbeafe';
            context.font = '600 ' + g.secondarySize + 'px system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
    ...
    function drawAnimatedLowerThird(context, item, frame) {
        ...
        if (item.phase === 'entering') {
            ...
            if (behavior.entrance === 'slide_left') { offsetX = -frame.width * 0.35 * (1 - progress); }
            if (progress >= 1) { item.phase = 'visible'; item.visibleAt = frame.now; transition('vh360:studio-overlay:transition-end', 'lowerThird', item); changed('program', 'lowerThird'); }
            ...
            if (behavior.exit === 'slide_left') { offsetX = -frame.width * 0.35 * progress; }
    ...
    root.addEventListener('vh360:studio:program-resolution-change', (event) => {
