...
class VH360_Studio_Bible_Importer {
    ...
    private function sanitize_meta( $m ) {
        return array( 'translation_key' => $this->limit_text( sanitize_key( isset( $m['translationKey'] ) ? $m['translationKey'] : '' ), 64 ), 'name' => $this->limit_text( sanitize_text_field( isset( $m['name'] ) ? $m['name'] : '' ), 200 ), 'abbreviation' => $this->limit_text( sanitize_text_field( isset( $m['abbreviation'] ) ? $m['abbreviation'] : '' ), 40 ), 'language' => $this->limit_text( sanitize_text_field( isset( $m['language'] ) ? $m['language'] : '' ), 40 ), 'source_name' => $this->limit_text( sanitize_text_field( isset( $m['sourceName'] ) ? $m['sourceName'] : '' ), 200 ), 'source_url' => esc_url_raw( isset( $m['sourceUrl'] ) ? $m['sourceUrl'] : '' ), 'license_name' => $this->limit_text( sanitize_text_field( isset( $m['licenseName'] ) ? $m['licenseName'] : '' ), 200 ), 'license_url' => esc_url_raw( isset( $m['licenseUrl'] ) ? $m['licenseUrl'] : '' ), 'attribution' => sanitize_textarea_field( isset( $m['attribution'] ) ? $m['attribution'] : '' ), 'attribution_required' => ! empty( $m['attributionRequired'] ) ? 1 : 0, 'dataset_version' => $this->limit_text( sanitize_text_field( isset( $m['datasetVersion'] ) ? $m['datasetVersion'] : '' ), 100 ) );
    }
