...
class VH360_Studio_Bible_Importer {
    ...
    public function process_batch( $id, $limit = null ) {
        ...
        $job = $this->get_job( $id );
        if ( $eof && ( $failed || (int) $job['rows_failed'] > 0 ) ) { $this->cleanup_staging( $job, false ); if ( file_exists( $job['source_file'] ) ) { wp_delete_file( $job['source_file'] ); } $this->release_job_lock( $id ); return $job; }
