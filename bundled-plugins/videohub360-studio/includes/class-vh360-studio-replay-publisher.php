...
class VH360_Studio_Replay_Publisher {
    ...
    public function __construct( VH360_Studio_Provider_Registry $registry, VH360_Studio_Recording_Jobs $jobs, VH360_Studio_Recording_Validator $validator, VH360_Studio_Recording_Chunks $chunks ) {
    ...
    public function prepare( array $job ) {
        ...
        $provider_id = sanitize_key( $job['storage_provider'] );
        ...
        $recording = array(
            'path'               => $job['local_temp_path'],
            'file_size'          => absint( $job['file_size'] ),
            'mime_type'          => $job['mime_type'],
            'assembled_checksum' => $job['assembled_checksum'],
        ...
        if ( empty( $recording['path'] ) || ! $this->chunks->is_path_in_base_directory( $recording['path'] ) || ! file_exists( $recording['path'] ) ) {
        ...
        return array(
            ...
            'provider_label'          => $providers[ $provider_id ]->get_label(),
            'job_status'              => $job['status'],
            'file_size'               => $recording['file_size'],
            'mime_type'               => $recording['mime_type'],
            'assembled_checksum'      => $recording['assembled_checksum'],
            'publish_provider_status' => $status,
            'message'                 => is_wp_error( $prepared ) ? $prepared->get_error_message() : __( 'Replay publishing bridge prepared.', 'videohub360-studio' ),
        );
    }
}
