...
class VH360_Studio_Video_Upload_REST_Controller {
    ...
    public function get( WP_REST_Request $request ) { $asset = $this->service->refresh_status( $request['uuid'] ); return $asset ? rest_ensure_response( $this->service->prepare_response( $asset ) ) : new WP_Error( 'not_found', __( 'Video asset not found.', 'videohub360-studio' ), array( 'status' => 404 ) ); }
