...
class VH360_Studio_User_Preferences {
    ...
    public static function allowed_overlay_modules() { return array( 'lower-thirds', 'bible', 'countdown' ); }
    public static function sanitize_overlay_modules( $modules ) {
        if ( ! is_array( $modules ) ) { return array(); }
        $allowed = self::allowed_overlay_modules(); $seen = array();
        foreach ( $modules as $module ) { $module = sanitize_key( $module ); if ( in_array( $module, $allowed, true ) ) { $seen[ $module ] = true; } }
        return array_values( array_filter( $allowed, static function( $module ) use ( $seen ) { return isset( $seen[ $module ] ); } ) );
    }
