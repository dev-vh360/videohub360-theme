...
if (!function_exists('videohub360_render_chat_container')) {
    function videohub360_render_chat_container($chat_placement, $is_user_logged_in, $user_avatar, $user_display_name, $user_logout_url, $can_moderate, $livestream_fields) {
        ...
        if ($can_moderate) {
            $chat_html .= '<button type="button" class="videohub360-moderation-panel-btn" id="vh360-moderation-panel-btn" title="' . esc_attr__('Moderation Panel', 'videohub360') . '">';
        ...
        if ($chat_placement === 'popup') {
            $chat_html .= '<button type="button" class="videohub360-chat-close-btn" id="vh360-chat-close-btn" title="' . esc_attr__('Close Chat', 'videohub360') . '" aria-label="' . esc_attr__('Close Chat', 'videohub360') . '">&times;</button>';
        }
        $chat_html .= '</div>';
        $chat_html .= '</div>';
        ...
        if ($is_user_logged_in) {
            ...
            $chat_html .= '<button type="button" class="videohub360-chat-logout-btn" data-logout-url="' . esc_url($user_logout_url) . '">' . esc_html__('Logout', 'videohub360') . '</button>';
        } else {
            $chat_html .= '<span>' . esc_html__('👤 Not logged in', 'videohub360') . '</span>';
        }
        $chat_html .= '</div>';
        $chat_html .= '</div>';
        $chat_html .= '</div>';
